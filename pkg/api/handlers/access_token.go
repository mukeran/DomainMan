...
func (AccessTokenHandler) Preload() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if v := db.Where("id = ?", accessTokenID).First(&accessToken); errors.Is(v.Error, gorm.ErrRecordNotFound) {
...
func (AccessTokenHandler) List() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := db.Select([]string{"id", "created_at", "updated_at", "name", "is_master", "can_issue", "issuer_id"}).Offset(int(offset)).Limit(int(limit))
		if issuer != 0 {
